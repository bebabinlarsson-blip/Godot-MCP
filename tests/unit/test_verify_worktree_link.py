...
worktree's distributable ``addons/godot_ai`` (or, on Windows, a directory junction).
...
``addons/`` tree) so the real worktree's link is never touched. POSIX-only —
...
def _make_sandbox(tmp_path: Path) -> Path:
    ...
    ``addons/`` and ``test_project/addons/godot_ai`` relative to that root.
    """
    root = tmp_path / "sandbox"
    (root / "script").mkdir(parents=True)
    (root / "addons" / "godot_ai").mkdir(parents=True)
    (root / "addons" / "godot_ai" / "plugin.gd").write_text(
...
def test_stale_real_dir_is_repaired_to_symlink(tmp_path: Path) -> None:
    ...
    assert link.resolve() == (root / "addons" / "godot_ai").resolve()
...
def test_healthy_symlink_is_left_intact(tmp_path: Path) -> None:
    ...
    root = _make_sandbox(tmp_path)
    link = root / "test_project" / "addons" / "godot_ai"
    link.symlink_to(Path("../../addons/godot_ai"))
