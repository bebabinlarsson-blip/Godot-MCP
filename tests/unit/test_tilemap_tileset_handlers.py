...
async def test_tilemap_place_tile_forwards_command_and_params():
    client = StubClient()
    runtime = DirectRuntime(registry=SessionRegistry(), client=client)
    ...
    assert client.calls[-1]["command"] == "tilemap_place_tile"
    assert client.calls[-1]["params"] == {
        "path": "/Main/Ground",
        "source_id": 1,
        "atlas_col": 2,
...
async def test_tilemap_rotate_cell_forwards_command_and_params():
    client = StubClient()
    runtime = DirectRuntime(registry=SessionRegistry(), client=client)
...
async def test_tilemap_flip_cell_forwards_command_and_params():
    client = StubClient()
    runtime = DirectRuntime(registry=SessionRegistry(), client=client)
...
async def test_tilemap_erase_cell_forwards_command_and_params():
    client = StubClient()
    runtime = DirectRuntime(registry=SessionRegistry(), client=client)

    await tilemap_handlers.tilemap_erase_cell(
        runtime,
        path="/Main/Ground",
        map_x=7,
        map_y=8,
    ...
    assert client.calls[-1]["command"] == "tilemap_erase_cell"
    assert client.calls[-1]["params"] == {
        "path": "/Main/Ground",
        "map_x": 7,
        "map_y": 8,
...
async def test_tilemap_get_cell_forwards_command_and_params():
    client = StubClient()
    runtime = DirectRuntime(registry=SessionRegistry(), client=client)

    await tilemap_handlers.tilemap_get_cell(
        runtime,
        path="/Main/Ground",
        map_x=7,
        map_y=8,
    ...
    assert client.calls[-1]["command"] == "tilemap_get_cell"
    assert client.calls[-1]["params"] == {
        "path": "/Main/Ground",
        "map_x": 7,
        "map_y": 8,
