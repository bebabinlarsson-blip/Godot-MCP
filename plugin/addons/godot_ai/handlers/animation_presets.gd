...
func preset_spin(params: Dictionary) -> Dictionary:
	...
	var handler = _h()
	if handler == null:
		return ErrorCodes.make_not_ready(ErrorCodes.SUB_EDITOR_UNAVAILABLE, "AnimationHandler has been disposed")

	var resolved := _resolve_preset_target(player_path, target_path)
	if resolved.has("error"):
		return resolved

	var player: AnimationPlayer = resolved.player
	var library: AnimationLibrary = resolved.library
	var created_library: bool = resolved.created_library
	var track_target: String = resolved.track_target
